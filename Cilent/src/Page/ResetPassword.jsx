...
axios.defaults.withCredentials=true;
  ...
  const navigate = useNavigate();
    ...
    const onsubmitEmail =async(e) =>{
      ...
      try {
        const {data}=await axios.post(backendUrl+'/api/auth/rest-otp',{
          email
        })
       ...
       <img onClick={()=>navigate('/')} src={logo} alt=""
        ...
        <form onSubmit={onsubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 test-sm'>
           <h1 className='text-white text-2xl font-semibold text-center mb-4'>Reset password</h1>
            <p className='text-center mb-6 text-indigo-300'>Enter your register email id.</p>
...
{!isotpSubmit && isEmailsent &&
     <form onSubmit={onSubmitOTp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 test-sm'>
            ...
            <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 font-semibold rounded-full'>Submit</button>
           </form>
}
           ...
           <h1 className='text-white text-2xl font-semibold text-center mb-4'>New password</h1>
            ...
            <div className='text-white mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                 < RiLockPasswordFill />
